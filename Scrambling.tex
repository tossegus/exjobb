...
The data processed by the DVB systems is sent in data packets. There exists three
different kinds of data packets, but only two of them are commonly used. The 
packets are called TS, PS and PES. TS is are used when data is being transmitted,
and PS is used when data is being stored. DVB uses itself of TS packets, while 
the data on DVDs are found in PS packets.
